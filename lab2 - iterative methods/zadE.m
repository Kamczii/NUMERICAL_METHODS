...
residuum = ones(N,1); 
ITER=1;
res_i = 1; %indeks residuum
%clock
tic
while norm(residuum) > 10^(-14)
    results = -D\(L+U)*results+D\b;
...
figure();
plot(arr, time);
title("Czas analizy Jacobi");
xlabel("Rozmiar macierzy");
ylabel("Czas [s]");
saveas(gcf, "zadE_184374_1.png");

figure();
plot(arr, iterations);
title("Liczba iteracji Jacobi");
xlabel("Rozmiar macierzy");
ylabel("Liczba iteracji");
saveas(gcf, "zadE_184374_2.png");
